...
window.onload = function() {
	...
	if (data) {
		...
		ship = ships[data.toTitleCase()];
		createItemDisplay(ship);
	} else {
		tableParse(ships);
...
function sortItems(sortBy) {
	...
	if (ships.Aerie[sortBy]) {
		for (ship in ships) {//create item list
			shipList.push(ships[ship]);
			if (!shipList[shipList.length - 1][sortBy]) {
				shipList[shipList.length - 1][sortBy] = 0;
			}
		}
	} else {
		for (ship in ships) {//create item list
			shipList.push(ships[ship]);
			if (!shipList[shipList.length - 1].attributes[sortBy]) {
				shipList[shipList.length - 1].attributes[sortBy] = 0;
			}
		}
	}
	...
	if (shipList[0][sortBy]) {

		if (typeof shipList[0][sortBy] == "number") {
			shipList.sort(function (a, b) {//sort numerical items
				if (reverse) {
			...
			shipList.sort(function (a, b) {//sort alphabetical items
				if (reverse) {
		...
		for (let i = 0; i < shipList.length; i++) {//append items
			if (shipList[i][sortBy]) {
				addTableItem(shipList[i]);
			}
		}
	} else {

		if (typeof shipList[0].attributes[sortBy] == "number") {
			shipList.sort(function (a, b) {//sort numerical items
				if (reverse) {
			...
			shipList.sort(function (a, b) {//sort alphabetical items
				if (reverse) {
					return (b.attributes[sortBy].toLowerCase().localeCompare(a.attributes[sortBy].toLowerCase()));
				} else {
					return (a.attributes[sortBy].toLowerCase().localeCompare(b.attributes[sortBy].toLowerCase()));
				}
			});
		}
...
function addTableItem(data, column1 = document.getElementById("select1").value, column2 = document.getElementById("select2").value) {
	var table = document.getElementById("mainTable");
	...
	name.onmousemove = function () {
		...
		if (imgPreview.src != "images/" + ships[this.innerText].sprite + ".png" || imgPreview.naturalWidth > 200 || imgPreview.naturalHeight > 200) {
			imgPreview.src = "images/" + ships[this.innerText].sprite + ".png";
	...
	if (data.attributes[column1] === 0) {
		var col1 = 0
	} else {
		var col1 = data.attributes[column1] || data[column1];
		if (typeof col1 == "number") {
			col1 = commafy(col1);
		}
	}
	firstCol.innerText = col1;
	if (data.attributes[column2] === 0) {
		var col2 = 0
	} else {
		var col2 = data.attributes[column2] || data[column2];
...
function createItemDisplay(data) {
	...
	image.src = "images/" + data.sprite + ".png";
	var style = "";
	image.onload = function() {
		if (this.naturalWidth < 200 && this.naturalHeight < 200) {
			style = "width: " + this.naturalWidth + "px; height: " + this.naturalHeight + "px; margin: " + (((200 - this.naturalHeight)/2) + 25) + "px 0 0 " + (((200 - this.naturalWidth)/2) + 25) + "px;";
		} else {
		...
		document.getElementById("itemPic").src = "images/" + data.sprite + ".png";
	...
	var barList = getData(data, stats, ["attributes"]);
...
function getData(data, dataFormat, subTags) {
	...
	for (let i = 0; i < dataFormat.group.length; i++) {//loop though main list
		for (let j = 0; j < dataFormat.group[i].length; j++) {//loop through sub lists
			dataTag = dataFormat.group[i][j];
			if (dataTag in subData) {
				...
				flip = dataFormat[dataTag] == "bad" ? true : false;
				percent = getPercent(dataFormat[dataTag + "_min"], dataFormat[dataTag + "_max"], barValue);
